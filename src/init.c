			...
			if((file_entry = readdir(tabdir)) != NULL) 
			{
				...
				while (fgets(buf, sizeof(buf), fp_x) != NULL) { lineNo++ ;
					...
					if(!strcmp(mainSEP, "BIN")) {
						...
						paramX->nfs = endLine ;
					}else if (!strcmp(mainSEP, "WAIT-TIME")) {  
						if (strcmp(endLine, "")) {
							paramX->wait_rt = (int*)endLine ;
						}
					} else if(!strcmp(mainSEP, "PID")) {    
						paramX->pid = endLine ;
						...
						if (!strcmp(endLine, ch->rlevel)) 
						{
							...
							if ( ((*paramX).pid != NULL) && (pid != paramX->pid) ) break;
							else if ( (*paramX).pid ){                                  /* is not recomendet */
								while( strcmp(paramX->pid, ch->pid) ){
									i =ch->next ;
								}
							}
