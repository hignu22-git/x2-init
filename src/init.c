					...
					if(!strcmp(mainSEP, "BIN")) {
						...
						if (!strcmp(endLine, ch->rlevel)) 
						{
							...
							if(paramX->onRestart) { /*Restart unit */
								 ...
								 *	non-kill    - if program killed - Restart 
								 *	console-log - depending on the console logs *(see CONSOLE-STAT )
								 */
								break_v == 1;
								char *cmdp = "ps -eo comm | grep ";
								if     ( !strcmp(paramX->onRestart,"non-kill"   ) ){
									pf  = popen( sprintf(cmd_str ,paramX->bin ,paramX->param ),"r");
									pss = popen( sprintf(cmd_st2 , cmdp       ,paramX->bin   ),"r");
									if (  !strcmp(pss, "")) {
										kill(paramX->pid ,SIGTERM );
										/* SETSIG(sa, SIGTERM, signal_handler, 0); */
								    }
									pclose(pf);
								}else if( !strcmp(paramX->onRestart,"console-log") ){	
									pf  = popen( sprintf(cmd_str ,paramX->bin ,paramX->param),"r");
